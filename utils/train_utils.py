...
def simclr_train(train_loader: DataLoader, model: Module, criterion: Module, optimizer: Optimizer, epoch: int) -> float:
    """ 
    ...
    for i, batch in enumerate(train_loader):
        ...
        b, c, h, w = images.size()
        input_ = torch.cat(
            [images.unsqueeze(1), images_augmented.unsqueeze(1)], dim=1)
        input_ = input_.view(-1, c, h, w)
        input_ = input_.cuda(non_blocking=True)
        targets = batch['target'].cuda(non_blocking=True)

        output = model(input_).view(b, 2, -1)
        loss:Tensor = criterion(output)
...
def scan_train(train_loader, model, criterion, optimizer, epoch, update_cluster_head_only=False):
    ...
    for i, batch in enumerate(train_loader):
        ...
        consistency_losses.update(
            np.mean([v.item() for v in consistency_loss]))
